...
class Phasor:
    ...
    def integrate_(pixel,lam,harm=1,ver=0):
        ...
        if ver==1:
            S_num = np.trapezoid(pixel_n*np.sin(harm*(lam)*(2*np.pi/(np.amax(lam)-np.amin(lam)))),lam,axis=-1)
            G_num = np.trapezoid(pixel_n*np.cos(harm*(lam)*(2*np.pi/(np.amax(lam)-np.amin(lam)))),lam,axis=-1)
            denom = np.trapezoid(pixel_n,lam)
        else:
            S_num = np.trapezoid(pixel_n*np.sin(harm*(lam-np.amin(lam))*(2*np.pi/(np.amax(lam)-np.amin(lam)))),lam,axis=-1)
            G_num = np.trapezoid(pixel_n*np.cos(harm*(lam-np.amin(lam))*(2*np.pi/(np.amax(lam)-np.amin(lam)))),lam,axis=-1)
            denom = np.trapezoid(pixel_n,lam)
    ...
    def __init__(self,  name, file, precomp_flag=False, harm=1,thresh=None,filt=False,mask=None,min_lambda=None,
        ...
        if file is None and data_arr is not None:
            ...
            self.file_ext = file_split[1]
            if file_split[1] in ['.czi','.tif']:

                try:
                    self.img = CziFile(file).scenes[0]
                except:
                    ...
                    if file_split[1] == '.czi':
                        if 'Z' in self.img.dims:
                            if crop_dims is not None:
                                self.data_cyx = self.img.asarray().astype(np.float32)[:,crop_dims[0]:crop_dims[1],crop_dims[2]:crop_dims[3],crop_dims[4]:crop_dims[5]]
                            else:
                                self.data_cyx = self.img.asarray().astype(np.float32)
                        else:
                            if crop_dims is not None:
                                self.data_cyx = self.img.asarray().astype(np.float32)[:,crop_dims[0]:crop_dims[1],crop_dims[2]:crop_dims[3]]
                            else:
                                self.data_cyx = self.img.asarray().astype(np.float32)

                        self.lambdas = np.array([float(f.split(' ')[0]) for f in list(self.img.channels)])
    ...
    def plot_phasor(self,ax1=None,cmap=cmap,p_max=1.0,plt_save=False):
        ...
        if plt_save:
            plt.savefig(self.name+'_phasor_plot.svg',dpi=600)
        return ax1
...
class PairMixture(Phasor):
    ...
    def calc_coloc(self,plot=False,ax1=None):
        ...
        self.coloc = np.trapezoid(h*bins[1:],bins[1:])
    ...
    def calc_pearson_mander(self,pcs,lims=[2,98],thresh_override=False, thresh_method = None):
        ...
        self.pearsons, _ = pearson_corr_coeff(t1,t2,mask=np.logical_and(mask1,mask2))
